...
target_compile_options(
  cc-compile-flags INTERFACE $<$<COMPILE_LANGUAGE:CXX>:-std=c++17 -W -Wall
                             -Werror>)
target_link_libraries(cc-compile-flags INTERFACE -Wl,--no-as-needed -ldl)
# add include directories
target_include_directories(cc-compile-flags BEFORE
...
function(cc_add_sanitizer_flag)
  ...
  if(SANITIZE_UNDEFINED)
    ...
    cc_target_compile_options(NAME ${_RULE_NAME} COPTS
...
function(cc_enable_profiling)
  ...
  if(USE_TIMEMORY)
    ...
    # find_package( timemory REQUIRED COMPONENTS headers compile-options
...
function(cc_target_compile_options)
  ...
  if(_RULE_COPTS)
    logger(UNIT "${_RULE_NAME}" LEVEL "DEBUG"
           "target_compile_options ${_RULE_COPTS}")
...
function(cc_target_link_options)
  ...
  if(_RULE_LINKOPTS)
    logger(UNIT "${_RULE_NAME}" LEVEL "DEBUG"
           "target_link_options ${_RULE_LINKOPTS}")
...
function(cc_target_link_libraries)
  ...
  if(_RULE_DEPS)
    logger(UNIT "${_RULE_NAME}" LEVEL "DEBUG"
           "target_link_libraries ${_RULE_DEPS}")
...
function(cc_add_executable)
  cmake_parse_arguments(
    ...
    "SRCS;HDRS;INCLUDES;DEFINES;COPTS;LINKOPTS;DEPS;TAGS" # multi value
                                                          # argumentes
    ${ARGN})
  # Sanity checks
  ...
  if(NOT _RULE_SRCS)
    message(
      FATAL_ERROR
        "cc_add_executable(NAME ${_RULE_NAME} ...) given with no sources")
  endif()
  ...
  if(_RULE_OUTPUT_NAME)
    set_target_properties(${_RULE_NAME} PROPERTIES OUTPUT_NAME
                                                   ${_RULE_OUTPUT_NAME})
  endif()
  if(_RULE_OUTPUT_DIR)
...
function(cc_add_library)
  ...
  if("${_RULE_NAME}" STREQUAL "")
    message(
      FATAL_ERROR "cc_add_library() must be called with argument NAME <name>")
  endif()
  ...
  elseif(_RULE_INTERFACE)
    ...
    message(
      FATAL_ERROR "cc_add_library ${_RULE_NAME} invoked without library type")
  endif()

  logger(UNIT ${_RULE_NAME} LEVEL "STATUS"
         "Adding library ${_RULE_NAME} ${_RULE_SRCS}")
